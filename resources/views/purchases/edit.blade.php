...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h4 class="fw-bold mb-1">Edit Purchase Order #{{ $purchase->purchase_number }}</h4>
        <p class="text-muted small mb-0">Modify purchase order details and line items</p>
    </div>
...
<form action="{{ route('purchases.update', $purchase) }}" method="POST" id="purchaseForm">
            ...
            <div class="row">
                ...
                <div class="col-md-4 mb-3">
                    ...
                    <input type="datetime-local" name="purchased_at" class="form-control" 
                           value="{{ old('purchased_at', \Carbon\Carbon::parse($purchase->purchased_at)->format('Y-m-d\TH:i')) }}" required>
                </div>
                <div class="col-md-4 mb-3">
                    <label class="form-label fw-semibold">Purchase Number</label>
                    <input type="text" name="purchase_number" class="form-control bg-light" value="{{ old('purchase_number', $purchase->purchase_number) }}" readonly tabindex="-1">
                    <div class="form-text">Auto-generated purchase number.</div>
                </div>
            </div>
        </div>
    </div>
                    ...
                    <tbody id="itemsTableBody">
                        @foreach($purchase->items as $index => $item)
                            <tr class="item-row">
                                ...
                                <td>
                                    <input type="number" step="0.01" min="0" name="items[{{ $index }}][unit_cost]" class="form-control cost-input" value="{{ $item->unit_cost }}" required>
                                </td>
                                <td>
                                ...
                                <td class="text-end pe-4">
                                    <button type="button" class="btn btn-sm btn-outline-danger remove-row-btn" {{ count($purchase->items) == 1 ? 'disabled' : '' }}>
            ...
            <div class="row justify-content-end">
                <div class="col-md-4">
                    <div class="d-flex justify-content-between mb-2">
                        ...
                        <span class="fw-semibold" id="subtotalDisplay">₹{{ number_format($purchase->subtotal, 2) }}</span>
                    </div>
                    <div class="d-flex justify-content-between mb-2">
                        <span class="text-muted">Total Tax:</span>
                        <span class="fw-semibold" id="taxDisplay">₹{{ number_format($purchase->tax_amount, 2) }}</span>
                    </div>
                    <hr class="my-2">
                    <div class="d-flex justify-content-between">
                        <span class="fw-bold fs-5">Grand Total:</span>
                        <span class="fw-bold fs-5 text-primary" id="grandTotalDisplay">₹{{ number_format($purchase->total_amount, 2) }}</span>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="d-flex justify-content-end gap-2 mb-5">
        ...
        <button type="submit" class="btn btn-primary px-5 fw-semibold">
            <i class="bi bi-check-lg me-1"></i> Update Purchase Order
        </button>
    </div>
</form>
@endsection
